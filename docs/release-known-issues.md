...
- 2026-05-26 发版记录：`v0.9.30` 继续上传无空格资产名 `Coding.Desktop.app.tar.gz`、`Coding.Desktop_0.9.30_aarch64.dmg` 和 `latest.json`。本次更新包大小 `11,198,995 bytes`，SHA256 为 `512b738d8a9f92afc66fe21187f9d8e64753085ef26159c7dfc2bed7e5500314`；DMG 大小 `12,474,171 bytes`，SHA256 为 `223455c35916b590e533e5bdce8e72995f1ed1e2620d3d560f2727c4346448bd`。本次系统代理为 `http://127.0.0.1:7897`，API HEAD 代理约 `0.48s`、直连约 `0.9-1.4s`，因此上传走系统代理 HTTP/1.1：更新包上传约 `25s`，DMG 上传约 `27s`，`latest.json` 上传约 `1s`。发布后 `releases/latest/download/latest.json` 返回 `0.9.30`，签名长度 `416`；DMG HEAD 约 `1.18s` 返回 `200`；更新包走系统代理完整下载约 `38.99s`，下载哈希和本地构建产物一致。本次误并行触发了多条重复下载验证，部分并发下载耗时约 `75-82s`，不作为单独链路结论。
